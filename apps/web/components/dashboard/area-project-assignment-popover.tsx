...
export type AreaProjectAssignmentPopoverProps = {
  isOpen: boolean;
  onClose: () => void;
...
export function AreaProjectAssignmentPopover({
  isOpen,
  onClose,
  projects,
  onAssign,
  onCreateAndAssign,
}: AreaProjectAssignmentPopoverProps) {
        ...
        <div className="pointer-events-auto w-[420px] bg-surface border border-border rounded-xl shadow-2xl flex flex-col overflow-hidden">
          <div className="px-5 py-4 border-b border-border flex items-center justify-between shrink-0">
            <h3 className="text-[14px] font-semibold text-foreground">
              {isCreatingProject ? 'Create New Project' : 'Add to Area'}
            </h3>
            <button
              onClick={onClose}
            ...
            <>
                    ...
                    {filteredProjects.map(p => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <FolderGit2 className={`w-4 h-4 shrink-0 ${selectedProjectIds.includes(p.id) ? 'text-primary' : 'text-muted-foreground'}`} />
              ...
              <div className="px-5 py-4 border-t border-border flex items-center justify-between shrink-0 bg-muted/30">
                ...
                <button 
                  ...
                  onClick={() => {
                    if (selectedProjectIds.length > 0) onAssign(selectedProjectIds);
                  }}
                >
                  Add to area
                </button>
