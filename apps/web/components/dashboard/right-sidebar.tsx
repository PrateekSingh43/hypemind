...
export function RightSidebar({
        ...
        setMessages([
          {
            ...
            role: "assistant",
            content:
              "I can help with workspace reasoning once your account and workspace are connected.",
          },
        ]);
        return;
      }

      const bootstrap = await api.get<BootstrapResponse>(
        `/workspaces/${workspaceId}/bootstrap`,
      );
      const inboxCount = bootstrap.data?.inboxCount ?? 0;
      const latestTitle =
        bootstrap.data?.recentItems?.find((item) => item.title?.trim())
          ?.title ?? null;
      ...
      setMessages([
        {
          ...
          role: "assistant",
          content:
            "I can still receive prompts, but context loading failed for this session.",
        },
      ]);
    }
  }, []);
  ...
  useEffect(() => {
    return subscribeToWorkspaceChange(() => {
      setSmartInsight("Syncing workspace context...");
      setMessages([]);
      void initializeContext();
    });
  }, [initializeContext]);
    ...
    <div className="flex h-full min-h-0 w-full flex-col border-l border-[#27282B] bg-[#151618]">
      ...
      <div className="shrink-0 px-3 pt-3 pb-2">
        <div className="rounded-lg bg-[#1C1D21] p-3">
          ...
          <p className="text-[13px] leading-relaxed text-[#C4C4C4]">
            {smartInsight}
          </p>
          <button
            type="button"
      ...
      <div className="flex min-h-0 flex-1 flex-col px-3">
        ...
        {chatError && (
          <p className="shrink-0 py-1 text-[11px] text-destructive">
            {chatError}
          </p>
        )}
      </div>
