...
export async function getTrashService(workspaceId: string): Promise<TrashItem[]> {
  ...
  const trash: TrashItem[] = [
    ...
    ...items.map((i) => ({ id: i.id, title: i.title || "Untitled", type: "ITEM" as const, deletedAt: i.deletedAt! })),
  ];

  return trash.sort((a, b) => b.deletedAt.getTime() - a.deletedAt.getTime());
}

export async function restoreTrashItemService(workspaceId: string, id: string, type: "AREA" | "PROJECT" | "ITEM") {
  if (type === "AREA") {
...
export async function permanentDeleteTrashItemService(workspaceId: string, id: string, type: "AREA" | "PROJECT" | "ITEM") {
  if (type === "AREA") {
